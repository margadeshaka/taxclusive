...
async function getSSMParameter(name: string): Promise<string> {
  return getCachedItem(`ssm:${name}`, async () => {
  ...
  return getCachedItem(
    ...
    async () => {
      try {
        ...
        const prerenderManifest = await getS3Object(
          bucketName,
          "current/.next/prerender-manifest.json"
        );
...
async function handleAPIRoute(
  ...
  try {
    ...
    const config = await getCachedItem("api-config", async () => ({
...
function getPageDescription(path: string): string {
  const descriptionMap: Record<string, string> = {
    ...
    "/about":
      "Learn about Taxclusive, our experienced team of chartered accountants and our commitment to excellence in financial services.",
    "/services":
      "Comprehensive CA services including tax planning, audit, compliance, GST, income tax, and financial advisory services.",
    "/contact":
      "Get in touch with Taxclusive for professional chartered accountancy services. Contact our expert CA team today.",
    "/blogs":
      "Stay updated with the latest tax news, regulations, and financial insights from our expert chartered accountants.",
    "/faq":
      "Find answers to frequently asked questions about our chartered accountancy services, tax compliance, and financial advisory.",
  };

  return (
    descriptionMap[path] ||
    "Professional chartered accountancy services by Taxclusive - your trusted CA firm."
  );
}
