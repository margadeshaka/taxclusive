...
export function initializeConfig(customConfig?: Partial<WebsiteConfiguration>) {
  return {
    ...
    validation: customConfig ? validateConfig(createConfig(customConfig)) : validateConfig(defaultConfig),
  };
}
...
export function createBusinessConfig(businessInfo: {
  ...
  return {
    ...
    description: businessInfo.description || `Professional services provided by ${businessInfo.name}`,
    contact: {
